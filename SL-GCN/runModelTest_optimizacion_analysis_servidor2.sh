...
declare -a lrs=(0.5 0.0005 0.0005 0.0005 0.0005 0.0005 0.0005 0.0005 0.0005)
...
for i in 0 # seed
do
  for j in 0 #8 0 2 # dataset-keypoint
  do 
      python main.py --seed $i --model_version ${model_version[j]} --experiment_name "results/${points[j]}/${datasets[j]}/wholepose-${datasets[j]}-s-$i" --database ${datasets[j]} --keypoints_model wholepose --base_lr ${lrs[j]} --keypoints_number ${points[j]} --num_epoch 3000  --mode_train "modelos_optimizados_early_stopping" --patience 10
